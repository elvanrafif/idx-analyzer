...
# ─── KONFIGURASI SESSION & USER-AGENT ───
# Ini diletakkan di luar route agar hanya dibuat satu kali saat app start
session = Session()
session.headers.update({
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Safari/537.36'
})

# ─── FRONTEND HTML (STRIPED) ───
HTML = '''<!DOCTYPE html>
...
@app.route('/')
def index():
    # Hanya satu fungsi index() untuk rute '/'
...
def analyze():
    ...
    try:
        # Menggunakan session global dengan User-Agent
...
if __name__ == '__main__':
    print("=" * 50)
    print("  📊 IDX STOCK ANALYZER — Server Running")
    print("  Port: 8080")
    print("=" * 50)
